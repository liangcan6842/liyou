...
from lib.other_tools.models import TestMetrics


class AllureFileClean:
    """allure 报告数据清除，提取业务需要的数据"""
    @classmethod
    def get_test_cases(cls) -> List:
        """获取所有allure报告中执行用例的情况"""
        #将所有数据都收集到files中
        files =[]
        for i in get_all_files(ensure_path_sep("\\report\\html\\data\\test_cases")):
